...
type ThreadReference = Reference & { number: number };
...
export default function ThreadPage() {
  ...
  // Extract and renumber references cumulatively across AI messages
  const threadReferences: ThreadReference[] = useMemo(() => {
    const refs: ThreadReference[] = [];
    let counter = 1;
    messages.forEach(m => {
      if (m.role !== 'ai' || !m.text) return;
      const text = m.text;
      const sectionMatch = text.match(/(?:References?|Sources?|Bibliography):[\s\S]*$/i);
      ...
      lines.forEach(line => {
        ...
        const content = m1 ? m1[1] : (m2 ? m2[2] : null);
        if (content) {
          refs.push({ text: content, messageId: m.id, number: counter++ });
        }
      });
    });
    return refs;
