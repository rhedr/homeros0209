...
type InsightsPanelProps = {
  ...
  threadReferences?: { text: string; messageId: string; number: number }[];
};
                            ...
                            {threadReferences && threadReferences.length > 0 ? (
                                <div className="space-y-2 text-sm text-muted-foreground">
                                  {threadReferences
                                    ...
                                    .map((ref, index) => (
                                      <div
                                        key={`${ref.messageId}-${ref.number}-${index}`}
                                        className="cursor-pointer hover:text-foreground transition-colors"
                                        onClick={() => onReferenceClick?.({ text: ref.text, messageId: ref.messageId })}
                                        role="button"
                                        tabIndex={0}
                                        onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') onReferenceClick?.({ text: ref.text, messageId: ref.messageId }) }}
                                      >
                                        <span className="text-foreground mr-2">{ref.number}.</span>
